...
export default [
  // UMD build for browser and direct script tag use (unminified)
  {
    ...
    output: {
      name: 'ClientDB', // The global variable name when loaded via script tag
      file: pkg.browser || 'dist/clientdb.umd.js', // Use pkg.browser if defined, or default
      format: 'umd',
      sourcemap: true,
      globals: {
  ...
  // Minified UMD build
  {
    ...
    output: {
      name: 'ClientDB',
      file: 'dist/clientdb.umd.min.js', // Separate minified UMD file
      format: 'umd',
      sourcemap: true,
      globals: {
  ...
  // ES module build (for bundlers like Webpack, Rollup, Parcel)
  {
    input: 'src/index.ts',
    output: {
      file: pkg.module || 'dist/clientdb.esm.js', // Use pkg.module if defined, or default
      format: 'es',
      sourcemap: true,
    },
  ...
  // CommonJS build (for Node.js environments)
  {
    input: 'src/index.ts',
    output: {
      file: pkg.main || 'dist/clientdb.cjs.js', // Use pkg.main if defined, or default
      format: 'cjs',
      sourcemap: true,
    },
